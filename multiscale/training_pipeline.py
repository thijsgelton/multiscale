...
def train(config: DictConfig) -> Optional[float]:
    ...
    log.info(f"Instantiating network architecture <{config.architecture._target_}>")
    architecture: nn.Module = hydra.utils.instantiate(config.architecture, _convert_="partial")

    # Init lightning model
    log.info(f"Instantiating model <{config.model._target_}>")
    model: LightningModule = hydra.utils.instantiate(config.model, net=architecture)
