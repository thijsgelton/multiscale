...
from models.components.base import ConvNormAct, delayed_fm
...
class MultiScaleMergeBlock(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        base_main = int(
            base_main * ((1 / 2) ** (layer_num + 1)))  # spatial size of target after curr level's encoder block
        ...
        self.reduce_fm_size_to_original.module[0].weight.data[0:in_channels, 0:in_channels, 0, 0] = torch.eye(
            in_channels)
        self.reduce_fm_size_to_original.module[0].bias.data.fill_(0)
        # add jitter
        epsilon = 1e-4
        normaldist = tdist.Normal(torch.tensor([0.0]), torch.tensor([epsilon]))
        jitter_weights = normaldist.sample(self.reduce_fm_size_to_original.module[0].weight.data.shape)[
            ..., 0]  # ignore dimension added at end
